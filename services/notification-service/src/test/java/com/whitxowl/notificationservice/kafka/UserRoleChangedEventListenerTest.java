...
class UserRoleChangedEventListenerTest {
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.kafka.consumer.group-id", () -> "notification-service-role-test");
    ...
    @Test
    void onUserRoleChanged_shouldCallSendRoleChanged_whenMessageReceived() {
        String userId = "user-role-1";
        ...
        kafkaTemplate.send("user.role-changed", userId, event);

        await()
                .atMost(10, TimeUnit.SECONDS)
                .untilAsserted(() ->
                        verify(notificationService).sendRoleChanged(event));
    }
